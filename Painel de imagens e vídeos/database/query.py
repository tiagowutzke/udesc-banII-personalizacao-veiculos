...
class Query:
    ...
    def select_all_joined(self, from_table, join_table, on, columns, how='INNER', where_clause=''):
        """ Select all rows from chosen table and columns
    ...
    def select_search_files(self, description=None, subject=None):
        where_clause = 'WHERE 1=1'
        if description:
            where_clause = f"{where_clause} AND images.description ilike '%{description}%' "
        if subject:
            where_clause = f"{where_clause} AND images.subject = {subject} "

        return self.select_all_joined(
            from_table='images',
            join_table='subjects',
            on=['subject', 'id'],
